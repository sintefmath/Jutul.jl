...
import Jutul: process_partition
...
@testset "Partitioning" begin
    ...
    @testset "process_partition" begin
        cart_mesh = CartesianMesh((5, 1))
        umesh = UnstructuredMesh(cart_mesh)
        for g in [cart_mesh, umesh]
            g = CartesianMesh((5,))
            p = [1, 1, 2, 1, 1]
            p_proc = process_partition(g, p)
            @test p_proc == [1, 1, 2, 3, 3] || p_proc == [3, 3, 2, 1, 1]
            p = [1, 1, 2, 3, 3]
            @test process_partition(g, p) == p
            @test !(process_partition(g, p) === p) # Should copy

            p_wgt = process_partition(g, p, weights = [1.0, 1.0, 1.0, 0.0])
            @test p_wgt == [1, 1, 2, 3, 4] || p_wgt == [1, 1, 2, 4, 3]
        end
    end
