...
function reorder_layout_layer!(layer, neighbors, ranks)
    old_rank = copy(ranks)
    sort!(
        layer; alg = MergeSort, by = node -> begin
...
function minimize_layout_crossings!(
        layers, previous_neighbors, next_neighbors;
        sweeps = 8
    )
...
function align_layout_layer!(
        coordinates, layer, neighbors, row_gap;
        relaxation = 0.65
    )
    ...
    for i in eachindex(targets)
        ...
        while length(block_values) > 1 &&
            ...
            value = (
                block_weights[end - 1] * block_values[end - 1] +
                    block_weights[end] * block_values[end]
            ) / weight
...
function variable_graph_layout(
        nodes, dependencies;
    ...
    minimize_layout_crossings!(
        layers, previous_neighbors,
        next_neighbors; sweeps = sweeps
    )
    y_coordinates = layout_y_coordinates(
        layers, previous_neighbors,
...
function draw_jutul_graph(
        nodes, dependencies, colors;
    ...
    positions, node_index, edge_paths = variable_graph_layout(
        nodes,
    ...
    for (_, deps) in enumerate(dependencies), _ in deps
        ...
        Makie.lines!(
            ax, path; color = edge_color, linewidth = edge_width,
        ...
        Makie.scatter!(
            ax, [arrow_position]; marker = :utriangle,
    ...
    function register_node_plot!(plot, node_index)
        ...
        return if hasproperty(plot, :plots)
        ...
        inspector_hover = if isnothing(tooltips)
            ...
            (inspector, _, _, _...) -> begin
                ...
                Makie.update_tooltip_alignment!(
                    inspector, mouse_position;
                    text = tooltips[i]
                )
                return true
            end
        end
        node_plot = Makie.textlabel!(
            ax, positions[i]; text = labels[i],
...
function variable_implementation(model, node)
    for collection in (
            model.primary_variables, model.secondary_variables,
            model.parameters,
        )
...
function Jutul.plot_variable_graph(model; kwargs...)
    ...
    Makie.Legend(
        ...
        [
            Makie.MarkerElement(
                color = palette[i], marker = :rect,
                markersize = Makie.Vec2f(28, 16), strokewidth = 1
            ) for i in 1:3
        ],
...
function Jutul.plot_model_graph(model::Jutul.MultiModel; kwargs...)
    ...
    for pair in model.cross_terms
        ...
        push!(
            dependencies[findfirst(==(model_nodes[source]), nodes)],
            equation_nodes[(target, target_equation)]
        )
        if Jutul.has_symmetry(pair.cross_term)
            push!(
                dependencies[findfirst(==(model_nodes[target]), nodes)],
