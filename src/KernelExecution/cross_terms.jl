...
@inline function (kernel::PreparedCrossTermPreparation)(index, dt)
    return Jutul.prepare_cross_term_in_entity!(
        index,
...
@inline function (kernel::PreparedCrossTermCache)(index, dt)
    return Jutul.update_prepared_cross_term_cache!(
        kernel.cache, index,
...
@inline function Base.getproperty(
        storage::PreparedCrossTermStorage,
        name::Symbol
    )
    if name === :evaluation
...
maybe_convert_cross_term_evaluation(
    plan::PreparedCrossTermEvaluation, context
) = nothing
...
function prepared_cross_term_cache_launch(
        cache, cross_term, states,
        models, equation
    )
...
function prepared_cross_term_cache_launch(
        cache::AbstractArray, cross_term,
...
function prepared_cross_term_target(
...
function prepared_cross_term_source(
        cache::GenericAutoDiffCache{
            <:Any, <:Any, ∂x, <:Any, <:Any, <:Any, <:Any, <:Any,
        },
        cross_term, equation, state_t, state0_t, state_s, state0_s,
...
function setup_cross_term_evaluation(
        ct_s, cross_term, equation,
...
function Jutul.update_cross_term!(
        ct_s::PreparedCrossTermStorage,
