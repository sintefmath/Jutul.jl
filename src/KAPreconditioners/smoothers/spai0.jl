...
@kernel function spai0_step_kernel!(
        dst, @Const(src), @Const(b), @Const(d),
...
@kernel function spai0_residual_step_kernel!(
        dst, @Const(src), @Const(residual),
        @Const(d), n
    )
    i = @index(Global)
    if i <= n
...
@kernel function spai0_zero_residual_step_kernel!(
        dst, @Const(residual),
        @Const(d), n
    )
    i = @index(Global)
...
@kernel function zero_spai0_residual_kernel!(
        x, residual, @Const(b), @Const(d),
        @Const(rp), @Const(cv),
        @Const(av), n
    )
    i = @index(Global)
...
function setup_smoother(
        A::StaticSparsityMatrixCSR{Tv}, config::SPAI0 = SPAI0();
    ...
    n = matrix_nrows(A)
    diagonal = zeros_reusing(
        old_diagonal, backend, Tv,
        n; reallocation_tracker = reallocation_tracker
    )
    if Tv <: Number
        temporary = zeros_reusing(
            old_temporary, backend, Tv, n;
            reallocation_tracker = reallocation_tracker
        )
        residual = zeros_reusing(
            old_residual, backend, Tv, n;
            reallocation_tracker = reallocation_tracker
        )
    else
        temporary = nothing
    ...
    state = SPAI0State(
        diagonal, temporary, residual, config,
...
function update_smoother!(state::SPAI0State, A::StaticSparsityMatrixCSR)
    ...
    kernel!(
        state.diagonal, A.rowptr, A.colval, A.nzval, smoother_damping(state),
        n; ndrange = n
    )
    return state
end
...
function apply_correction!(x, state::SPAI0State, residual)
    ...
    axpy!(
        x, state.temporary, one(smoother_damping(state)), backend,
        state.block_size
    )
    return x
end
...
function smooth_result!(
        x, A::StaticSparsityMatrixCSR, b, state::SPAI0State, steps::Int;
    ...
    for _ in start:steps
        step!(
            dst, src, b, state.diagonal, A.rowptr, A.colval, A.nzval,
            n; ndrange = n
        )
        src, dst = dst, src
    end
...
function smooth_once_to!(
        dst::Vector, src::Vector,
        A::StaticSparsityMatrixCSR{Tv, Ti, <:Vector, <:Vector, <:Vector},
        b::Vector, state::SPAI0State{D}
    ) where {Tv, Ti, D <: Vector}
    return spai0_step_cpu!(dst, src, A, b, state.diagonal)
end

function zero_smooth_residual!(
        x, residual, A::StaticSparsityMatrixCSR, b,
        state::SPAI0State
    )
    n = matrix_nrows(A)
    ...
    kernel!(
        x, residual, b, state.diagonal, A.rowptr, A.colval, A.nzval,
        n; ndrange = n
    )
    return residual
end
