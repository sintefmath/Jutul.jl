function setup_smoother(
    ...
    state = GaussSeidelState(
        A, inverse_diagonal, correction, residual,
...
function gauss_seidel_sweep!(
        x, A, b, inverse_diagonal, damping,
        rows
    )
...
function gauss_seidel_correction!(
        x, correction, A, residual,
...
function gauss_seidel_smooth!(
        x, A, b, state, steps, rows;
    ...
    if !isnothing(residual) && !zero_initial
        gauss_seidel_correction!(
            x, state.correction, A, residual,
...
function smooth_level!(
    ...
    return gauss_seidel_smooth!(
        x, A, b, state, steps, 1:state.n;
