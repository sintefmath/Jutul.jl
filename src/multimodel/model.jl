...
function synchronize_backend_increment_to_host!(storage, key)
    host = host_evaluation_entry(storage, key)
    if isnothing(host)
        return storage
    end
...
function synchronize_backend_residual_to_host!(storage, key)
    host = host_evaluation_entry(storage, key)
    if isnothing(host)
        return storage
    end
...
@inline function (kernel::PreparedCrossTermPreparation)(index, dt)
    return prepare_cross_term_in_entity!(index,
...
@inline function (kernel::PreparedCrossTermCache)(index, dt)
    return update_prepared_cross_term_cache!(kernel.cache, index,
...
function prepared_cross_term_target(cache::GenericAutoDiffCache{
        ...
        models) where ∂x
    states = (
        local_ad(state_t, 1, ∂x), local_ad(state0_t, 1, ∂x),
        as_value(state_s), as_value(state0_s))
...
function prepared_cross_term_source(cache::GenericAutoDiffCache{
        ...
        models) where ∂x
    states = (
        as_value(state_t), as_value(state0_t),
        local_ad(state_s, 1, ∂x), local_ad(state0_s, 1, ∂x))
    return prepared_cross_term_cache_launch(
        cache, cross_term, states, models, equation)
end
...
function update_cross_terms!(storage, model::MultiModel, dt;
    ...
    for index in eachindex(model.cross_terms)
        ...
        if is_match
            ...
            if isnothing(host) && haskey(storage, :cross_term_evaluations)
                evaluation = storage.cross_term_evaluations[index]
            else
                evaluation = nothing
            end
...
function transfer_cross_term_evaluation!(storage, model::MultiModel)
    ...
    for index in host.cross_term_evaluation.host
        ...
        backend_copyto!(destination.target, source.target)
...
function update_cross_term!(ct_s, ct::CrossTerm, eq, storage_t, storage_s,
        model_t, model_s, dt,
        evaluation::PreparedCrossTermEvaluation)
...
function update_cross_term!(ct_s, ct::CrossTerm, eq, storage_t, storage_s,
        model_t, model_s, dt, evaluation = nothing)
    state_t = evaluation_state(storage_t)
    state0_t = evaluation_state0(storage_t)
...
function update_linearized_system!(storage, model::MultiModel, executor = default_executor();
    ...
    zero_jacobian_entries!(model, storage, executor)
