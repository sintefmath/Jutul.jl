...
function initialize_variable_value!(state, model, pvar, symb, val::Union{AbstractJutulStorage, AbstractDict}; need_value = true, T = float_type(model.context))
    if haskey(val, symb)
        value = val[symb]
...
function initialize_parameter_value!(parameters, data_domain, model, param, symb, initializer::Union{AbstractJutulStorage, AbstractDict}; kwarg...)
