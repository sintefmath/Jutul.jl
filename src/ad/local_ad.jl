...
@inline function Base.getindex(state::LocalStateAD, s::Symbol)
    return Base.getproperty(state, s)
end
...
@inline function local_ad(state, index, ad_tag)
    return local_state_ad(state, index, ad_tag)
end

@inline function local_ad(state, index, ad_tag, symbol)
    return local_state_ad(state, index, ad_tag, symbol)
end
...
function Base.show(io::IO, t::MIME"text/plain", x::LocalStateAD{T, I, E}) where {T, I, E}
    return print(io, "Local state for $(unpack_tag(E)) -> $(getfield(x, :index)) with fields $(keys(getfield(x, :data)))")
end
